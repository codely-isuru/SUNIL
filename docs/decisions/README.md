...
| [031](ADR-031-parked-turns-and-continuations.md) | **A turn that hits ASK_USER ends `parked`**; approval runs a persisted system continuation (consume CAS, single-use binding). Adds the third `outcome` to the C5 envelope | Proposed (V2 Phase 0, 2026-09-10) · **Amended once** (consume ownership → Tool Manager, fix round 2026-09-10) |
| [032](ADR-032-v2-deployment-topology-and-ports.md) | **V2 dev topology:** app on host, infra in Compose; fixed loopback ports — **per Amendment 1:** web 3001, api 8000, postgres 5433, litellm 4000, n8n 5680 on the host (in-network `postgres:5432`/`litellm:4000`/`n8n:5678`); compose at `infra/docker-compose.yml`, profiles deferred | Proposed (V2 Phase 0, 2026-09-10) · **Amended once** (ports/location/profiles to reality + CI parity check, fix round 2026-09-10) |
...
| ADR-031 | Amendment 1 — consume ownership: the Tool Manager (not the continuation executor) issues the `approved→consumed` CAS via `ApprovalsService.consume`; startup reconciliation rules added (QA B3, Security items 1–2, Phase 0 fix round) | 2026-09-10 |
| ADR-032 | Amendment 1 — host ports reconciled to build-machine reality (postgres **5433**, n8n **5680**, web **3001**; in-network unchanged); compose lives at `infra/docker-compose.yml`; `infra`/`full` profiles deferred until the api container exists; CI parity check required (QA B6 = Security B1, Phase 0 fix round) | 2026-09-10 |
