"""V2 PORT NOTE (Stream A, task/S-A-tools): this module is M1's
`apps/api/sunil/tools/github/projection.py`, ported BYTE FOR BYTE with only this
paragraph added. It was M1's headline security control, its behaviour is proven
by M1's review history, and it has no V1 imports and no database dependency — so
re-deriving it would have meant re-deriving the reviews too. Its property test
(no content can change the delimiter count) is ported alongside it in
`tests/unit/tools/test_github_native.py`. Where V2's contract DID change (C1
§4's closed error set, `ToolResultMeta` on every result), the change is in
`adapter.py`, not here. M1's original docstring follows.
